...
BEGIN
	...
	SELECT array_agg(5) INTO position_ids;
	FOREACH pos_id IN ARRAY position_ids LOOP
		...
		------ PERFORM f_t_adtf_dist_gps_pos(pos_id); 
		...
		PERFORM f_t_adtf_dist_pos(pos_id); 
		...
		PERFORM f_t_adtf_dist_pos_dist_s_rnd1(pos_id); 
		-- Aggregate to 10 Hz time (and minimise distance)
		PERFORM f_t_adtf_dist_pos_dist_m_rnd1(pos_id); 
		...
		PERFORM f_t_adtf_full_dist_m_rnd1_pxx(pos_id); 
		-- Join time-aggregated arrival measures with vehicle data and round names
		PERFORM f_t_adtf_full_dist_s_rnd1_pxx(pos_id); 
	END LOOP;
END $do$;